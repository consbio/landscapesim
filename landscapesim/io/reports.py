    ...
    with open(file, 'r') as sheet:
        ...
        for row in reader:
            r = StateClassSummaryReportRow.objects.create(
                ...
                stratum=Stratum.objects.filter(name__exact=row['StratumID'], project=s.project).first(),
                stateclass=StateClass.objects.filter(name__exact=row['StateClassID'], project=s.project).first(),
            ...
            if len(secondary_stratum) > 0:
                r.secondary_stratum = SecondaryStratum.objects.filter(name__exact=secondary_stratum, project=s.project).first()
                r.save()
    return report
        ...
        for row in reader:
            r = TransitionByStateClassSummaryReportRow.objects.create(
                ...
                transition_type=TransitionType.objects.filter(
                    name__exact=row['TransitionTypeID'], project=s.project).first(),
                stateclass_src=StateClass.objects.filter(
                    name__exact=row['StateClassID'], project=s.project).first(),
                stateclass_dest=StateClass.objects.filter(
                    name__exact=row['EndStateClassID'], project=s.project).first(),
                amount=float(row['Amount'])
            )
            secondary_stratum = row['SecondaryStratumID']
            if len(secondary_stratum) > 0:
...
def create_state_attribute_summary(s, file):
    report = StateAttributeSummaryReport.objects.create(scenario=s)
    with open(file, 'r') as sheet:
        reader = csv.DictReader(sheet)
        for row in reader:
...
def create_transition_attribute_summary(s, file):
    report = TransitionAttributeSummaryReport.objects.create(scenario=s)
    with open(file, 'r') as sheet:
        reader = csv.DictReader(sheet)
        for row in reader:
            r = TransitionAttributeSummaryReportRow.objects.create(
                ...
                transition_attribute_type=TransitionAttributeType.objects.filter(
                    name__exact=row['TransitionAttributeTypeID'], project=s.project).first(),
                age_min=default_int(row['AgeMin']),
                age_max=default_int(row['AgeMax']),
                amount=float(row['Amount']),
            )
